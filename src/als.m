...
% err_als_old     =     (res(:)')*res(:);
err_als_old = frob(Y-cpdgen({A_aux,B_aux,C_aux}),'squared');
% Squared norm of Y
...
%                             sum(sum((C_aux'*C_aux).*temp_BA));
    err_als           =     frob(Y-cpdgen({A_aux,B_aux,C_aux}),'squared');
    
    % Evaluation of the relative error decrease
    diff_err_filt(1:end-1)  =     diff_err_filt(2:end);
    diff_err_filt(end)=     abs((err_als-err_als_old)/err_als_vec(1));
...
% Normalization to have unitary first rows in A and B
C_aux =     C_aux.*repmat(A_aux(1,:).*B_aux(1,:),dim(3),1);
A_aux =     A_aux.*repmat(1./A_aux(1,:),dim(1),1);
B_aux =     B_aux.*repmat(1./B_aux(1,:),dim(2),1);
